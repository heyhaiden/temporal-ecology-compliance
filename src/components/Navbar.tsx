...
// Memoized dropdown menu items to prevent unnecessary re-renders
const SolutionsDropdownContent = memo(() => (
...
SolutionsDropdownContent.displayName = "SolutionsDropdownContent";

// Memoized mobile menu to prevent unnecessary re-renders
const MobileMenu = memo(({ isAuthenticated }: { isAuthenticated: boolean }) => (
  <Sheet>
...
const Navbar = () => {
  ...
  useEffect(() => {
    // Use a more efficient authentication check
  ...
  return (
    <nav className="fixed top-0 w-full bg-white/80 backdrop-blur-md border-b z-50 pr-[var(--removed-body-scroll-bar-size)]">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Link to="/solutions" className={cn("text-sm font-medium transition-colors hover:text-emerald-600 flex items-center gap-1", location.pathname.startsWith("/solutions") ? "text-emerald-600" : "text-gray-600")}>
